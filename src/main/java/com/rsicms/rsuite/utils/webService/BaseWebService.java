...
public abstract class BaseWebService
    extends DefaultRemoteApiHandler implements WebServiceUtilsConstants {
   ...
   */
  protected static int getNotificationDurationInSeconds(
      ExecutionContext context) {
    return ConfPropsUtils.getPropertyAsInt(
        context.getConfigurationProperties(),
        PROP_NAME_NOTIFICATION_DURATION_IN_SECONDS,
        DEFAULT_NOTIFICATION_DURATION_IN_SECONDS);
  }

  /**
   * Find out if less severe messages should be displayed alongside the more severe messages.
   * 
   * @param context
   * @return True when less severe messages should be included; else, false.
   */
  protected static boolean alsoDisplayLessSevereMessages(
      ExecutionContext context) {
    return ConfPropsUtils.getPropertyAsBoolean(
        context.getConfigurationProperties(),
        PROP_NAME_ALSO_DISPLAY_LESS_SEVERE_MESSAGES,
        DEFAULT_ALSO_DISPLAY_LESS_SEVERE_MESSAGES);
  }
   ...
   * that incorporates the success* parameters, and when it doesn't want to refresh any
  ...
  protected RemoteApiResult getWebServiceResponse(
    ...
    return getWebServiceResponse(
        ...
        user,
        opResult,
        new ArrayList<String>(), // No need to refresh RSuite object when downloading a file.
  ...
  protected RemoteApiResult getWebServiceResponse(
      ...
      User user,
      OperationResult opResult,
      List<String> refreshUponSuccessIds,
      boolean refreshChildrenUponSuccess,
      UserInterfaceAction successUserInterfaceAction,
    ...
    if (opResult.hasFailures()) {
      ...
      if (fileDownloadResult != null) {
        ...
        closeMessage(
            buf,
            opResult);
        RestResult restResult = getInfoResult(
            context,
            user,
            buf.toString(),
            refreshUponSuccessIds,
            refreshChildrenUponSuccess,
            successMessage);
   ...
   */
  protected void addWarningMessages(
      StringBuilder buf,
      boolean includeLessSevereMessages,
      OperationResult opResult) {
    addMessages(
        buf,
        opResult.getWarningMessages(),
        "Warnings: ");
    if (includeLessSevereMessages) {
      addMessages(
          buf,
          opResult.getInfoMessages(),
          "Info: ");
    }
  }
   ...
   * @param opResult the Operation Result.
   */
  protected void addFailureMessages(
      StringBuilder buf,
      boolean includeLessSevereMessages,
      OperationResult opResult) {
    addMessages(
        buf,
        opResult.getFailureMessages(),
        "Errors: ");
    if (includeLessSevereMessages) {
      addMessages(
          buf,
  ...
  protected static void addMessages(
      StringBuilder buf,
      List<? extends ProcessMessage> messages,
      String messageType) {
   ...
   *        Subclasses may override. If ignored, the successMessage parameter is expected to be
   ...
   */
  protected RestResult getInfoResult(
      ExecutionContext context,
      User user,
      String msg,
      List<String> refreshUponSuccessIds,
    ...
    log.info(new StringBuilder(user.getUserId()).append(
        ": ").append(
        successMessage));

    RestResult webServiceResult = getNotificationResult(
        context,
        successMessage,
        getWebServiceLabel());
   ...
   */
  protected static RestResult getNotificationResult(
      ExecutionContext context,
      String message,
      String title) {
    ...
    notification.addProperty(
        NotificationAction.PROPERTY_DURATION,
        getNotificationDurationInSeconds(context));
